    ...
    initHttp: function(config){
        return new Promise(function (resolve, reject) {
            ...
            SERVER.httpServer.listen(config.httpPort, config.hostname, () => {
                console.log(`Server running at http://${config.hostname}:${config.httpPort}/ with ${config.envName} mode.`);
                resolve(config);
            });
        })
    },
        ...
        return new Promise(function (resolve, reject) {
            if (config.httpsKey && config.httpsCert) {
                ...
                SERVER.httpsServer.listen(config.httpsPort, config.hostname, () => {
                    console.log(`Server running at https://${config.hostname}:${config.httpsPort}/ with ${config.envName} mode.`);
                });
            } else {
